...
class Config(object):
    """Load KEPPNet global, model, and dataset configuration files.

    Configuration files live in the repository-level ``configs/`` directory.
    Explicit values supplied through ``config_dict`` or command-line overrides
    take precedence over the JSON defaults.
    """
    ...
    def _load_internal_config(self):
        config_dir = self._repository_config_dir()
        config_path = os.path.join(config_dir, 'config.json')
        self.internal_config_dict = read_json_data(config_path)

    @staticmethod
    def _repository_config_dir():
    ...
    def _build_path_config(self):
        ...
        model_config_file = os.path.join(config_dir, "model/{}.json".format(model_name))
        best_config_file = os.path.join(config_dir, "best_config/{}_{}.json".format(model_name, dataset_name))
        dataset_config_file = os.path.join(config_dir, "dataset/{}.json".format(dataset_name))
