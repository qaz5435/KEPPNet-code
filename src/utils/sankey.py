...
def get_data_trace(links, node_df, height, width, fontsize=6):
    ...
    layout = dict(
        ...
        font=dict(
            size=max(fontsize, 10), family='Arial',
        )
    )
...
def get_fromated_network(links, high_nodes_df, col_name, remove_others):
    ...
    # ensure all 'others*' nodes appearing in links have a color
    try:
        nodes_in_links = set(list(links['source'].unique()) + list(links['target'].unique()))
        missing_others = [n for n in nodes_in_links if isinstance(n, str) and ('others' in n) and (n not in node_colors_dict)]
        for n in missing_others:
            # default grey with alpha depending on remove_others flag
            node_colors_dict[n] = (232, 232, 232, 0.5 if not remove_others else 0.0)
    except Exception:
        pass
    ...
    # ensure every layer index exists for layout spacing
    min_l, max_l = int(node_layers_df.layer.min()), int(node_layers_df.layer.max())
    for l in range(min_l, max_l + 1):
        tag = f'others{l}'
        if tag not in node_layers_df.index:
            node_layers_df.loc[tag] = l
    ...
    # per-layer clipping to reduce single-edge dominance and drop tiny edges
    try:
        p = 0.95
        linkes_filtred_encoded_df['value'] = linkes_filtred_encoded_df.groupby('layer')['value'].transform(
            lambda x: np.minimum(x, np.quantile(x, p))
        )
        min_edge = linkes_filtred_encoded_df.groupby('layer')['value'].transform('max') * 0.02
        linkes_filtred_encoded_df = linkes_filtred_encoded_df[linkes_filtred_encoded_df['value'] >= min_edge]
    except Exception:
        pass
    ...
    node_colors_list = []
    for l in all_node_labels:
        # fallback color for any residual missing nodes
        node_colors_list.append(node_colors_dict.get(l, (232, 232, 232, 0.5)))
...
def run(result_path, number_layer=7):
    ...
    # Prefer per-layer Top-K exported by interpretation if available
    try:
        topk_path = join(RESULT_PATH, 'extracted/topk_summary.csv')
        if os.path.exists(topk_path):
            df_topk = pd.read_csv(topk_path)
            df_topk = df_topk.sort_values(['layer', 'rank'])
            high_nodes = list(df_topk['node'].unique())
        else:
    ...
    links_df = get_links()
    # optional blacklist for noisy/broad pathways
    blacklist_keywords = ['ADME', 'Infection', 'Infectious', 'secondary', 'Generic Transcription Pathway']
    def not_blacklisted(name):
        for kw in blacklist_keywords:
            if isinstance(name, str) and kw.lower() in name.lower():
                return False
        return True
    links_df = links_df[links_df['source'].apply(not_blacklisted) & links_df['target'].apply(not_blacklisted)]
