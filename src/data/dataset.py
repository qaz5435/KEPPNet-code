...
default_structured_path = join(data_path, 'structured', 'prad')
processed_path = join(default_structured_path, 'processed')
...
def load_data(filename, selected_genes=None, processed_dir=processed_path):
    filename = join(processed_dir, filename)
    ...
    if 'response' in cached_data:
        ...
        labels = get_response(processed_dir)
...
def load_TMB(filename=gene_final_no_silent_no_intron, processed_dir=processed_path):
    x, response, samples, genes = load_data(filename, processed_dir=processed_dir)
    x = np.sum(x, axis=1)
    x = np.array(x)
    x = np.log(1. + x)
    n = x.shape[0]
...
def load_CNV_burden(filename=gene_final_no_silent_no_intron, processed_dir=processed_path):
    x, response, samples, genes = load_data(filename, processed_dir=processed_dir)
    x = np.sum(x, axis=1)
    x = np.array(x)
    x = np.log(1. + x)
...
def load_data_type(data_type='gene', cnv_levels=5, cnv_filter_single_event=True,
                   mut_binary=False, selected_genes=None, processed_dir=processed_path):
    print('loading {}'.format(data_type))
    if data_type == 'TMB':
        x, response, info, genes = load_TMB(gene_important_mutations_only, processed_dir)
    if data_type == 'mut_no_silent_no_intron':
        x, response, info, genes = load_data(gene_final_no_silent_no_intron, selected_genes, processed_dir)
        if mut_binary:
            print('mut_binary = True')
            x[x > 1.] = 1.

    if data_type == 'mut_important':
        x, response, info, genes = load_data(gene_important_mutations_only, selected_genes, processed_dir) # put processed data_access about prostate genes into 14k genes
        if mut_binary:
            print('mut_binary = True')
            x[x > 1.] = 1.

    if data_type == 'mut_important_plus_hotspots':
        x, response, info, genes = load_data(gene_important_mutations_only_plus_hotspots, selected_genes, processed_dir)

    if data_type == 'mut_hotspots':
        x, response, info, genes = load_data(gene_hotspots, selected_genes, processed_dir)

    if data_type == 'truncating_mut':
        x, response, info, genes = load_data(gene_truncating_mutations_only, selected_genes, processed_dir)
        if mut_binary:
            print('mut_binary = True')
            x[x > 1.] = 1.
    ...
    if data_type == 'cnv':
        x, response, info, genes = load_data(cnv_filename, selected_genes, processed_dir)
        if cnv_levels == 3:
            print('cnv_levels = 3')
    ...
    if data_type == 'gene_expression':
        x, response, info, genes = load_data(gene_expression, selected_genes, processed_dir)

    if data_type == 'fusions':
        x, response, info, genes = load_data(fusions_filename, None, processed_dir)

    if data_type == 'cnv_burden':
        x, response, info, genes = load_data(cnv_burden_filename, None, processed_dir)
        # x.loc[:, :] = 0.

    if data_type == 'fusion_genes':
        x, response, info, genes = load_data(fusions_genes_filename, selected_genes, processed_dir)
        # x.loc[:,:]=0.

    return x, response, info, genes


def get_response(processed_dir=processed_path):
    print('loading response from %s' % response_filename)
    labels = pd.read_csv(join(processed_dir, response_filename))
...
class BnetData():
    ...
    def __init__(self, config:Config):
        ...
        self.processed_dir = config['processed_dir'] or processed_path
        self.splits_dir = config['splits_dir'] or join(default_structured_path, 'splits')
        ...
        if type(data_type) == list:
            ...
            for t in data_type:
                x, y, rows, cols = load_data_type(
                    t, cnv_levels, cnv_filter_single_event, mut_binary,
                    selected_genes, self.processed_dir
                )
            ...
            x, y, rows, cols = load_data_type(
                data_type, cnv_levels, cnv_filter_single_event, mut_binary,
                selected_genes, self.processed_dir
            )
        ...
        if selected_samples is not None:
            selected_samples_file = join(self.processed_dir, selected_samples)
